...
<script>
    ...
    @foreach($records as $row => $kentang)
        ...
        var ke1 = {{($recorddetail->k1)}};
        var ke2 = {{($recorddetail->k2)}};
        var ke3 = {{($recorddetail->k3)}};
        var ke_tma = {{($recorddetail->k_tma)}};
        var mongol = {{($kentang->nilai)}};
        
        var qnya = ((ke1 * (Math.pow(mongol, 2)))+(ke2 * (mongol))+ke3 ).toFixed(2);
        {
            nilainya === 0;
        }
