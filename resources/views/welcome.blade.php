...
<script>
    ...
    @foreach($ars as $kentang => $record)
        ...
        var newData = { idnya:jujukan_id, lat: lats, lng: longs, name: '{{$record->pos_name}}',vmax:valuemax,vmin:valuemin,daterecord:'{{$record->tlocal}}', intivalue:valuenya, kentang:jujukan_id,debit:10,debitmax:10,debitmin:10,k1:k1,k2:k2,k3:k3,k_tma:k_tma }
    ...
    //         callback(data.value,data.max_value,data.min_value); // Call the callback function with the fetched data
    //     },
    ...
    //     return function(value,max_value,min_value) {
    //         var marker = L.marker([data.lat, data.lng], { icon: redIcon }).bindPopup("<hr style='margin-bottom:5px;margin-top:5px;color:black;'><div class='text-primary' style='margin-bottom:5px;font-style:italic;font-size:12px;'>Nama Pos :<b>" + data.name + "</b></div><div class='text-primary' style='margin-bottom:5px;font-style:italic;font-size:12px;'>Koordinat : LS " + data.lat + ", BT " + data.lng + "</div><table class='table table-bordered' style='margin-bottom:5px;'><thead class='colorthead thead-dark'><tr><th scope=col' style='vertical-align: text-bottom;'>Parameter</th><th class='w-auto' style='vertical-align: text-top;'>Nilai</th><th scope='col' style='vertical-align: text-top;'>Data Max Sesaat</th><th scope='col' style='vertical-align: text-top;'>Data Min Sesaat</th></tr></thead><tbody><tr><td>TMA</td><td>"+value+" m</td><td class='text-white' style='background:#a31919' >"+max_value+" m</td><td class='text-white' style='background:#ff8c40;'>"+min_value+" m</td></tr><tr><td>Debit</td><td> "+(data.k1 * Math.pow(value, 2) + (data.k2 * value) + data.k3 ).toFixed(2)+" m<sup>3</sup>/s</td><td class='text-white' style='background:#a31919' >"+(data.k1 * Math.pow(max_value, 2) + (data.k2 * max_value) + data.k3).toFixed(2)+" m<sup>3</sup>/s</td><td class='text-white' style='background:#ff8c40;'>"+(data.k1 * Math.pow(min_value, 2) + (data.k2 * min_value) + data.k3).toFixed(2)+" m<sup>3</sup>/s</td></tr></tbody></table><div class='mt-1'><div class='text-danger' style='margin-bottom:15px;font-style:italic;font-size:12px;'>Data Terakhir : " + data.daterecord + " &nbsp <i class='bx bxs-calendar'></i></div><a class='btn btn-sm btn-secondary text-light' href='{{ url('/hardwaretable/') }}/"+data.kentang+"'>> check detail </a>", { 'className': 'custom-popup', closeButton: false }).on('mouseover', function() {
    //             this.openPopup();
    //         }).openPopup();
    //         marker.addTo(contoh);
    //     };
    ...
    // // Loop through data_example
    // for (var i = 0; i < data_example.length; i++) {
    //     (function(index) {
    //         // Call check function for each data_example
    //         check(data_example[index].idnya, createMarker(data_example[index]));
    //     })(i);
    // }
...
// NEW FUNCTION TO CALL AJAX
// >>>>>>>>>> ============================================================================================================================== <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<

    const contoh = L.layerGroup();
    function createMarkerWithAjax(data) {
        var marker = L.marker([data.lat, data.lng], { icon: redIcon }).bindPopup("", { 'className': 'custom-popup', closeButton: false }).on('mouseover', function() {
            // Open popup when mouse hovers over the marker
            this.openPopup();
            // Call the AJAX request when the popup is opened
            var popupContent = "<div class='d-flex'><div class='loading'></div> Loading . . . &nbsp&nbsp </div>";

            marker.getPopup().setContent(popupContent);

            $.ajax({
                url: 'http://sisirumba.pusair-pu.go.id/test_sisirumba/public/nilaipilihan/' + data.idnya,
                method: 'GET',
                dataType: 'json',
                beforeSend: function () {
                    console.log('Fetching data...');
                ...
                success: function(response) {
                    ...
                    var popupContent = "<hr style='margin-bottom:5px;margin-top:5px;color:black;'><div class='text-primary' style='margin-bottom:5px;font-style:italic;font-size:12px;'>Nama Pos :<b>" + data.name + "</b></div><div class='text-primary' style='margin-bottom:5px;font-style:italic;font-size:12px;'>Koordinat : LS " + data.lat + ", BT " + data.lng + "</div><table class='table table-bordered' style='margin-bottom:5px;'><thead class='colorthead thead-dark'><tr><th scope=col' style='vertical-align: text-bottom;'>Parameter</th><th class='w-auto' style='vertical-align: text-top;'>Nilai</th><th scope='col' style='vertical-align: text-top;'>Data Max Sesaat</th><th scope='col' style='vertical-align: text-top;'>Data Min Sesaat</th></tr></thead><tbody><tr><td>TMA</td><td>"+response.value+" m</td><td class='text-white' style='background:#a31919' >"+response.max_value+" m</td><td class='text-white' style='background:#ff8c40;'>"+response.min_value+" m</td></tr><tr><td>Debit</td><td> "+(data.k1 * Math.pow(response.value, 2) + (data.k2 * response.value) + data.k3 ).toFixed(2)+" m<sup>3</sup>/s</td><td class='text-white' style='background:#a31919' >"+(data.k1 * Math.pow(response.max_value, 2) + (data.k2 * response.max_value) + data.k3).toFixed(2)+" m<sup>3</sup>/s</td><td class='text-white' style='background:#ff8c40;'>"+(data.k1 * Math.pow(response.min_value, 2) + (data.k2 * response.min_value) + data.k3).toFixed(2)+" m<sup>3</sup>/s</td></tr></tbody></table><div class='mt-1'><div class='text-danger' style='margin-bottom:15px;font-style:italic;font-size:12px;'>Data Terakhir : " + data.daterecord + " &nbsp <i class='bx bxs-calendar'></i></div><a class='btn btn-sm btn-secondary text-light' href='{{ url('/hardwaretable/') }}/"+data.kentang+"'>> check detail </a>";
                    marker.getPopup().setContent(popupContent);
                },
                error: function(jqXHR, textStatus, errorThrown) {
                    console.error('There was a problem with the fetch operation:', errorThrown);
    ...
    // Loop through data_example and create markers with attached AJAX request
    for (var i = 0; i < data_example.length; i++) {
        createMarkerWithAjax(data_example[i]);
    }
