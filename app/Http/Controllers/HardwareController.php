...
class HardwareController extends Controller
{
    ...
    public function Penampang($id)
    {
        $records = Hardware::join('trs_raw_d_gpa', 'trs_raw_d_gpa.kd_hardware', '=', 'mst_hardware.kd_hardware')
            ...
            ->select(
                ...
                DB::raw('(mst_hardware.pos_img)'),
                DB::raw('(mst_hardware.foto_pos)'),
                DB::raw('(mst_hardware.plant)'),
                DB::raw('(mst_hardware.kd_kecamatan)'),
                DB::raw('(mst_hardware.kd_provinsi)'),
                DB::raw('(mst_hardware.kd_kabupaten)'),
                DB::raw('(mst_hardware.kd_desa)'),
                DB::raw('(mst_hardware.elevasi)'),
                DB::raw('(mst_hardware.pendiri)'),
                DB::raw('(mst_hardware.thn_dibangun)'),
                DB::raw('(mst_hardware.wilayah_sungai)'),
                DB::raw('(mst_hardware.nomor_pos)'),
                // DB::raw('(mst_hardware.kd_kecamatan)'),
                // DB::raw('(mst_hardware.kabupaten)'),
                // DB::raw('(mst_hardware.provinsi)'),
                // DB::raw('(mst_hardware.elevasi)'),
            )    
            ->orderBy('trs_raw_d_gpa.tlocal', 'desc')->first();

            return view('hardwaredetail',compact('records'));
        // return $records;
    }
    ...
    public function HardwareDetailCCTV($id)
    {
        ...
        // $arrayimg = CCTV::all();
        $arrayimg = CCTV::where('img_num',$recorddetail->cctv)->Latest()->take(3)->get();
        // return $arrayimg;
        if (!isset($recordcctv)){
        ...
        // return $records;
        return view('hardwaredetailcctv',compact('records','chance','recorddetail','awal','akhir','pilihannya','arrayimg'));
    }
    ...
    public function SelectCCTVFromDateRange(Request $request,$id)
    {
        ...
        // return $arrayimg; 
        // return $arrayimg;
        return view('hardwaredetailcctv',compact('chance','recorddetail','awal','akhir','pilihannya','arrayimg'));
    }
